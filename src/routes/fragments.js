...
router.post('/', rawBody(), async (req, res) => {
  try {
    if (!Buffer.isBuffer(req.body)) {
      ...
      return res
        .status(415)
        .json({ status: 'error', error: { code: 415, message: 'unsupported Content-Type or empty body' } });
    }
    ...
    if (!Fragment.isSupportedType(type)) {
      ...
      return res
        .status(415)
        .json({ status: 'error', error: { code: 415, message: `unsupported Content-Type: ${type}` } });
    }
    ...
    return res
      .status(500)
      .json({ status: 'error', error: { code: 500, message: 'internal server error' } });
  }
});
...
router.get('/', async (req, res) => {
  try {
    ...
    if (expand === '1' || expand === 'true') {
      ...
      return res.status(200).json({ status: 'ok', fragments: list.map((f) => f.toJSON()) });
    }
...
router.get('/:id', async (req, res) => {
  try {
    ...
    if (!frag) {
      return res.status(404).json({ status: 'error', error: { code: 404, message: 'fragment not found' } });
    }
    const data = await frag.getData();
    if (!data) {
      return res.status(404).json({ status: 'error', error: { code: 404, message: 'fragment data missing' } });
    }
...
router.put('/:id', rawBody(), async (req, res) => {
  try {
    ...
    if (!Buffer.isBuffer(req.body)) {
      return res.status(400).json({ status: 'error', error: { code: 400, message: 'missing body' } });
    }
    ...
    if (current !== incoming) {
      return res.status(400).json({
        status: 'error',
        error: { code: 400, message: `content type mismatch: existing=${current} new=${incoming}` },
      });
    }
